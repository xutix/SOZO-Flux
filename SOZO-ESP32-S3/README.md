...
电脑、手机和设备连接同一个路由器后，优先在浏览器打开 <http://sozo-esp32.local>。如果 Windows 或路由器没有启用 mDNS 解析，再使用串口或路由器客户端列表中的 IP，例如：<http://192.168.1.125>。

如果设备无法连接已保存的路由器，它会在约 30 秒后自动回到 `SOZO-SETUP` 配网模式。正常控制页的 **Configure Wi-Fi** 按钮会清除当前凭据并重新进入配网模式。
...
正常联网后，在电脑或手机的同一局域网中打开 <http://sozo-esp32.local>。
