...
本工程基于 PlatformIO 与 Arduino framework，适配 ESP32-S3-WROOM-1-N8R8（8MB Flash、8MB OPI PSRAM）。它作为 Flux Hub 维护权威空间场景，并支持 INMP441 拾音、手机网页控制、音乐律动和一个默认 144 颗且可在网页灯光节点页调整的可选本地 WS2812 灯光节点。
...
空间页显示当前空间场景、亮度、跟随节点应用进度和 Hub 拾音状态。选择某个灯效后，只显示该灯效相关的颜色模式和参数：
...
| `POST /api/lighting` | 保存权威空间场景的 `effect`、亮度、颜色及该灯效的参数；仅接受 11 种设备灯效 |
